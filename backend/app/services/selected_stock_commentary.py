...
from app.services.excess_return import (
    scan_stocks,
    SAMPLE_STOCKS,
    scan_overseas_stocks,
...
def generate_selected_stock_commentary(
    ...
    """
    국내 초과수익률 스캐닝 -> 임계치 초과 종목 필터링 -> 종목별 뉴스기반 코멘터리 생성
    """
    ...
    for stock in target_stocks:
        ...
        performance_desc = (
            "선방(초과수익)" if stock["excess_return_pct"] > 0
            else "부진(초과손실)" if stock["excess_return_pct"] < 0
            else "예상과 유사한 흐름"
        )
        actual_direction = (
            "상승" if stock["stock_return_pct"] > 0
            else "하락" if stock["stock_return_pct"] < 0
            else "보합"
        )

        prompt = f"""당신은 금융시장을 분석해 투자자에게 설명하는 애널리스트입니다.

{stock_name}은(는) 오늘 실제로 {stock['stock_return_pct']}% {actual_direction}했습니다.
{index_name} 지수는 {stock['index_return_pct']}% 움직였고, 베타({stock['beta']})를 감안한 예상 움직임은 {stock['expected_return_pct']}%였습니다.
즉 지수 요인을 감안했을 때 예상보다 {performance_desc}했으며, 순수 종목 고유 초과수익률은 {stock['excess_return_pct']}%p 입니다.
...
def generate_overseas_selected_commentary(
    threshold_pct: float = 1.0,
    top_n: int = 5,
    index_name: str = "나스닥",
) -> list:
    """
    해외 초과수익률 스캐닝 -> 임계치 초과 종목 필터링 -> 종목별 뉴스기반 코멘터리 생성
    (국내 generate_selected_stock_commentary와 동일 구조, 데이터소스만 해외)
    """
    scan_results = scan_overseas_stocks(stock_dict=SAMPLE_OVERSEAS_STOCKS, index_name=index_name)
...
if __name__ == "__main__":
    print("국내 선별종목 AI 코멘터리 생성 중...\n")
    domestic_results = generate_selected_stock_commentary(threshold_pct=1.0, top_n=5)

    if not domestic_results:
        print("국내: 임계치를 초과한 종목이 없습니다.")
    else:
        for r in domestic_results:
            print(f"\n{'=' * 60}")
            print(f"[국내] [{r['stock_name']}] {r['stock_return_pct']}% (지수대비 {r['excess_return_pct']}%p)")
            print("=" * 60)
            print(r["commentary"])

    print("\n\n해외 선별종목 AI 코멘터리 생성 중...\n")
    overseas_results = generate_overseas_selected_commentary(threshold_pct=1.0, top_n=5)

    if not overseas_results:
        print("해외: 임계치를 초과한 종목이 없습니다.")
    else:
        for r in overseas_results:
            print(f"\n{'=' * 60}")
            print(f"[해외] [{r['stock_name']}] {r['stock_return_pct']}% (지수대비 {r['excess_return_pct']}%p)")
            print("=" * 60)
            print(r["commentary"])